...
const SEED_EMPLOYEES = [
  { id: 1, name: "أحمد محمود", role: "كاشير", hired: "2025-01-01", salary: 2500, bonus: 300, deduct: 0, status: "مدفوع" },
  { id: 2, name: "سالم الزبيدي", role: "مشرف ألعاب", hired: "2025-03-15", salary: 3000, bonus: 500, deduct: 0, status: "مدفوع" },
  { id: 3, name: "فارس علي", role: "فني صيانة", hired: "2025-06-01", salary: 2800, bonus: 0, deduct: 200, status: "معلق" },
  { id: 4, name: "ريم صالح", role: "محاسب", hired: "2024-09-01", salary: 3200, bonus: 0, deduct: 0, status: "مدفوع" },
];
...
function NakheelApp() {
  // session state — تبقى الجلسة محفوظة حتى تسجيل الخروج
      ...
      <div style={{ position: "fixed", top: 0, right: 0, width: isTab ? 260 : 225, height: "100vh", background: `linear-gradient(180deg,${C.grn} 0%, ${C.dark ? "#0a0906" : "#0e3318"} 100%)`, overflowY: "auto", zIndex: 100, display: "flex", flexDirection: "column",
        ...
        <div style={{ padding: ".85rem .95rem", borderTop: "1px solid rgba(201,168,76,.2)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            ...
            <button onClick={() => setUser(null)} title="خروج" style={{ background: "none", border: "none", color: "rgba(255,255,255,.5)", cursor: "pointer", fontSize: 16 }}>⎋</button>
          </div>
        </div>
      </div>
...
function FirstSetup({ settings = {}, onCreate }) {
  const [f, setF] = useState({ name: "", username: "admin", clubName: settings.clubName || "نادي النخيل" });
  ...
  const create = () => {
    ...
    const admin = {
      id: 1, name: f.name.trim(), username: f.username.trim(), role: "مدير", shift: "—", active: true,
      perms: { invoices: true, discounts: true, cancel: true, reports: true, customers: true, prices: true, purchases: true, inventory: true, salaries: true },
      pages: {},
    };
    onCreate(admin);
...
function Login({ users, onLogin, settings = {} }) {
  ...
  const submit = (u) => {
    const usr = u || selected;
    if (!usr) { setErr("اختر المستخدم أولاً"); return; }
    onLogin(usr); // demo: any password works
  };
  ...
  return (
    <div dir="rtl" style={{ fontFamily: "'Tajawal',sans-serif", minHeight: "100vh", background: "radial-gradient(circle at 30% 20%, #1a5c2e 0%, #14431f 45%, #0a2712 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700;900&display=swap" rel="stylesheet" />
      <div style={{ background: "#fff", borderRadius: 22, padding: "2.2rem 2rem", width: 440, maxWidth: "95vw", boxShadow: "0 24px 70px rgba(0,0,0,.4)" }}>
        ...
        {!selected ? (
          ...
          <div>
            ...
            <label style={{ fontSize: 11, color: C.mt, fontWeight: 600 }}>كلمة المرور</label>
            <input type="password" autoFocus value={pwd} onChange={(e) => setPwd(e.target.value)} onKeyDown={(e) => e.key === "Enter" && submit()} placeholder="••••••••"
              style={{ width: "100%", marginTop: 4, marginBottom: 16, fontSize: 13, border: `0.5px solid ${C.bc}`, borderRadius: 10, padding: ".6rem .8rem", background: C.crm, fontFamily: "inherit", outline: "none" }} />
        ...
        <div style={{ fontSize: 10.5, color: C.mt, marginTop: 18, textAlign: "center", lineHeight: 1.7 }}>
          ...
          <span style={{ fontSize: 10 }}>وضع العرض: أي كلمة مرور تعمل</span>
        </div>
      </div>
    </div>
  );
...
function Modal({ title, onClose, children, width = 540 }) {
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()} style={{ position: "fixed", inset: 0, background: "rgba(10,30,15,.45)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 12 }}>
      <div style={{ background: C.cd, borderRadius: 16, padding: "1.4rem 1.5rem", width, maxWidth: "96vw", maxHeight: "90vh", overflowY: "auto" }}>
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 11, marginBottom: "1.1rem" }}>
        <KCard label="إجمالي المبيعات" value={fmt(monthSales)} sub="دينار ليبي" bar="#1a8c3e" delta={{ up: true, text: "+12.4%" }} />
        <KCard label="عدد الفواتير" value={invoices.length} bar="#2a78d6" />
        <KCard label="متوسط الفاتورة" value={invoices.length ? fmt(Math.round(monthSales / invoices.filter(i => i.status === "مدفوعة").length || 1)) : 0} sub="د.ل" bar={C.gold} />
      </div>
      <Card>
...
function CustomerDetail({ ctx, customer, onClose }) {
  ...
  const receivePayment = () => {
    ...
    setInvoices(iv => iv.map(x => settledIds.includes(x.id) ? { ...x, status: "مدفوعة" } : x));
...
function Expenses({ ctx }) {
  const { expenses, setExpenses, user, showToast } = ctx;
  const [modal, setModal] = useState(false);
  const [f, setF] = useState({ date: todayISO(), cat: "أجار", desc: "", amount: "", pay: "نقداً" });
  const save = () => {
    if (!f.desc.trim() || !f.amount) { showToast("أدخل الوصف والمبلغ"); return; }
    setExpenses(e => [{ id: e.length + 1, date: f.date, cat: f.cat, desc: f.desc, amount: parseFloat(f.amount), pay: f.pay, by: user.name.split(" ")[0] }, ...e]);
    showToast("تمت إضافة المصروف"); setModal(false); setF({ date: todayISO(), cat: "أجار", desc: "", amount: "", pay: "نقداً" });
  };
  const byCat = (c) => expenses.filter(e => e.cat === c).reduce((s, e) => s + e.amount, 0);
  const CAT_TONE = { "أجار": "b", "مرتبات": "gold", "كهرباء": "r", "صيانة": "g" };
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 11, marginBottom: "1.1rem" }}>
        ...
        <KCard label="المرتبات" value={fmt(byCat("مرتبات"))} sub="موظفين" bar={C.gold} />
        <KCard label="الكهرباء" value={fmt(byCat("كهرباء"))} bar={C.red} />
        <KCard label="إجمالي المصاريف" value={fmt(expenses.reduce((s, e) => s + e.amount, 0))} sub="د.ل" bar={C.purp} />
      </div>
      <Card>
        <CardHead title="سجل المصاريف" />
        <Table cols={[{ h: "التاريخ", w: "16%" }, { h: "الفئة", w: "16%" }, { h: "الوصف", w: "30%" }, { h: "المبلغ", w: "16%" }, { h: "الدفع", w: "14%" }, { h: "بواسطة", w: "8%" }]}
          rows={expenses.map(e => [arDate(e.date), <Badge tone={CAT_TONE[e.cat] || "g"}>{e.cat}</Badge>, e.desc, fmt(e.amount) + " د.ل", e.pay, e.by])} />
      </Card>
      {modal && <Modal title="إضافة مصروف" onClose={() => setModal(false)} width={460}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <Field label="التاريخ"><Inp type="date" value={f.date} onChange={e => setF({ ...f, date: e.target.value })} /></Field>
          <Field label="الفئة"><Sel value={f.cat} onChange={e => setF({ ...f, cat: e.target.value })}>{["أجار", "مرتبات", "كهرباء", "صيانة", "أخرى"].map(c => <option key={c}>{c}</option>)}</Sel></Field>
          <Field label="الوصف" full><Inp value={f.desc} onChange={e => setF({ ...f, desc: e.target.value })} /></Field>
          <Field label="المبلغ (د.ل)"><Inp type="number" value={f.amount} onChange={e => setF({ ...f, amount: e.target.value })} /></Field>
...
function Salaries({ ctx }) {
  const { employees, setEmployees, showToast } = ctx;
  const [modal, setModal] = useState(false);
  const [f, setF] = useState({ name: "", role: "", salary: "", bonus: "", deduct: "" });
  const save = () => {
    if (!f.name.trim() || !f.salary) { showToast("أدخل الاسم والراتب"); return; }
    setEmployees(e => [...e, { id: e.length + 1, name: f.name, role: f.role || "موظف", hired: todayISO(), salary: parseFloat(f.salary), bonus: parseFloat(f.bonus) || 0, deduct: parseFloat(f.deduct) || 0, status: "معلق" }]);
    showToast("تمت إضافة الموظف"); setModal(false); setF({ name: "", role: "", salary: "", bonus: "", deduct: "" });
  };
  const net = (e) => e.salary + e.bonus - e.deduct;
  const totalNet = employees.reduce((s, e) => s + net(e), 0);
  const togglePaid = (id) => setEmployees(es => es.map(e => e.id === id ? { ...e, status: e.status === "مدفوع" ? "معلق" : "مدفوع" } : e));
  return (
    <>
      <PageTop title="مرتبات العمال" action={<Btn gold onClick={() => setModal(true)}>+ إضافة موظف</Btn>} />
      <Card>
        <CardHead title="قائمة الموظفين" />
        <Table cols={[{ h: "الاسم", w: "18%" }, { h: "المنصب", w: "15%" }, { h: "التعيين", w: "13%" }, { h: "الراتب", w: "12%" }, { h: "المكافآت", w: "10%" }, { h: "الخصومات", w: "10%" }, { h: "الصافي", w: "12%" }, { h: "الحالة", w: "10%" }]}
          rows={employees.map(e => [e.name, e.role, arDate(e.hired), fmt(e.salary) + " د.ل", e.bonus ? fmt(e.bonus) : "—", e.deduct ? fmt(e.deduct) : "—", <span style={{ fontWeight: 600, color: C.grn2 }}>{fmt(net(e))} د.ل</span>, <span onClick={() => togglePaid(e.id)} style={{ cursor: "pointer" }}><Badge tone={e.status === "مدفوع" ? "g" : "a"}>{e.status}</Badge></span>])} />
        <div style={{ marginTop: ".75rem", paddingTop: ".65rem", borderTop: `0.5px solid ${C.bc}`, display: "flex", justifyContent: "space-between", fontSize: 12.5, color: C.mt }}><span>إجمالي المرتبات الشهرية</span><span style={{ fontWeight: 700, color: C.ink }}>{fmt(totalNet)} د.ل</span></div>
      </Card>
      {modal && <Modal title="إضافة موظف" onClose={() => setModal(false)} width={460}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <Field label="الاسم" full><Inp value={f.name} onChange={e => setF({ ...f, name: e.target.value })} /></Field>
          <Field label="المنصب"><Inp value={f.role} onChange={e => setF({ ...f, role: e.target.value })} placeholder="كاشير" /></Field>
          <Field label="الراتب الأساسي"><Inp type="number" value={f.salary} onChange={e => setF({ ...f, salary: e.target.value })} /></Field>
          <Field label="المكافآت"><Inp type="number" value={f.bonus} onChange={e => setF({ ...f, bonus: e.target.value })} /></Field>
          <Field label="الخصومات"><Inp type="number" value={f.deduct} onChange={e => setF({ ...f, deduct: e.target.value })} /></Field>
        </div>
        <div style={{ display: "flex", gap: 8 }}><Btn gold onClick={save} style={{ flex: 1, justifyContent: "center" }}>✓ حفظ</Btn><Btn onClick={() => setModal(false)}>إلغاء</Btn></div>
      </Modal>}
    </>
...
function Reports({ ctx }) {
  const { invoices, purchases, expenses, products, totals } = ctx;
  ...
  const cfg = useMemo(() => {
    if (type === "sales") {
      ...
      return { title: "تقرير المبيعات", summary: [["إجمالي المبيعات", fmt(totals.revenue) + " د.ل"], ["عدد الفواتير", invoices.length], ["متوسط الفاتورة", "297 د.ل"]],
        thead: ["رقم", "الزبون", "المصدر", "التفاصيل", "الدفع", "الإجمالي"], tbody: rows.map(i => ["#" + i.id, i.customer, i.source, i.details, i.pay, fmt(i.total) + " د.ل"]) };
    }
    if (type === "purchases") return { title: "تقرير المشتريات", summary: [["إجمالي المشتريات", fmt(purchases.reduce((s, p) => s + p.total, 0)) + " د.ل"], ["عدد الطلبات", purchases.length], ["الموردون", ctx.suppliers.length]], thead: ["رقم", "المورد", "المنتجات", "الدفع", "الإجمالي"], tbody: purchases.map(p => ["#" + p.id, p.supplier, p.items, p.pay, fmt(p.total) + " د.ل"]) };
    if (type === "profit") return { title: "تقرير الأرباح والخسائر", summary: [["الإيرادات", fmt(totals.revenue) + " د.ل"], ["المصاريف", fmt(totals.expTotal) + " د.ل"], ["صافي الربح", fmt(totals.profit) + " د.ل"]], thead: ["البند", "القيمة"], tbody: [["إجمالي الإيرادات", fmt(totals.revenue) + " د.ل"], ["إجمالي المصاريف", fmt(totals.expTotal) + " د.ل"], ["صافي الربح", fmt(totals.profit) + " د.ل"], ["هامش الربح", (totals.revenue ? (totals.profit / totals.revenue * 100).toFixed(1) : 0) + "%"]] };
    if (type === "expenses") return { title: "تقرير المصاريف", summary: [["إجمالي المصاريف", fmt(totals.expTotal) + " د.ل"], ["عدد البنود", expenses.length], ["أكبر بند", "المرتبات"]], thead: ["التاريخ", "الفئة", "الوصف", "المبلغ"], tbody: expenses.map(e => [arDate(e.date), e.cat, e.desc, fmt(e.amount) + " د.ل"]) };
    // products
    const rows = products.filter(p => !cat || p.cat === cat);
    return { title: "تقرير مبيعات المنتجات", summary: [["عدد المنتجات", rows.length], ["إيراد الألعاب", "58,200 د.ل"], ["إيراد الكافيه", "26,120 د.ل"]], thead: ["المنتج", "القسم", "الباركود", "سعر البيع", "المخزون"], tbody: rows.map(p => [p.name, CATS[p.cat], p.bc, fmt(p.sell) + " د.ل", p.stock !== null ? p.stock : "خدمة"]) };
  }, [type, cat, invoices, purchases, expenses, products, totals, ctx.suppliers.length]);
...
function Users({ ctx }) {
  ...
  const [sel, setSel] = useState(users.find(u => u.role === "بائع")?.id || users[0].id);
  const [modal, setModal] = useState(false);
  const [f, setF] = useState({ name: "", username: "", role: "بائع", shift: "صباحي" });
  ...
  const addUser = () => {
    ...
    setUsers(us => [...us, { id: Math.max(0, ...us.map(x => x.id)) + 1, name: f.name, username: f.username, role: f.role, shift: f.shift, active: true, perms, pages: {} }]);
    showToast("تمت إضافة المستخدم"); setModal(false); setF({ name: "", username: "", role: "بائع", shift: "صباحي" });
  };
  ...
  return (
    <>
      ...
      {modal && <Modal title="إضافة مستخدم جديد" onClose={() => setModal(false)} width={460}>
        ...
        <div style={{ fontSize: 11, color: C.mt, marginBottom: 10 }}>سيُمنح المستخدم صلاحيات افتراضية وكل القوائم ظاهرة — عدّلها بعد الإضافة.</div>
